...
OUTPUT_DIR = "output/gdrn/dft_labor"
INPUT = dict(
    DZI_PAD_SCALE=1.5,
    TRUNCATE_FG=False,
    CHANGE_BG_PROB=0.5,
...
SOLVER = dict(
    IMS_PER_BATCH=config.TRAIN_GDRNPP_BATCH_SIZE, #48
    TOTAL_EPOCHS=40,  # 30
...
DATALOADER = dict(
    ...
    NUM_WORKERS=config.TRAIN_GDRNPP_DATALOADER_WORKERS,
...
MODEL = dict(
    ...
    POSE_NET=dict(
        ...
        XYZ_ONLINE=True, # True
        NUM_CLASSES=len(config.MODELS_NAME),
        BACKBONE=dict(
            FREEZE=False,
            PRETRAINED="timm",
            INIT_CFG=dict(
