...
model.head.num_classes = 15

train.init_checkpoint = config.TRAIN_INIT_CHECKPOINT

# datasets
DATASETS.TRAIN = [config.DATASET_TRAIN] # noch unklar, refers to core/gdrn_modelling/datasets/...py -> SPLITS_DATA Zeile 300
DATASETS.TEST = [config.DATASET_TEST] # noch unklar, wahrscheinlich nur der Name

TEST = dict(EVAL_PERIOD=(1,))
...
optimizer = L(Ranger)(
    ...
    lr=config.TRAIN_YOLOX_OPTIMIZER_LR,  # bs=64
    # momentum=0.9,
    weight_decay=config.TRAIN_YOLOX_OPTIMIZER_WEIGHT_DECAY,
    # nesterov=True,
)

train.total_epochs = config.TRAIN_YOLOX_EPOCHS
train.no_aug_epochs = config.TRAIN_YOLOX_NO_AUGH_EPOCHS
...
dataloader.test = [
    L(build_yolox_test_loader)(
        ...
        total_batch_size=config.TRAIN_YOLOX_DATALOADER_TEST_BATCHSIZE,
        # total_batch_size=64,
        num_workers=config.TRAIN_YOLOX_DATALOADER_TEST_WORKERS,
        pin_memory=True,
    )
