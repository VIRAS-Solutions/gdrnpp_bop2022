...
PROJ_ROOT = osp.normpath("/gdrnpp_bop2022")
...
#DATASETS_ROOT = osp.normpath("/gdrnpp_bop2022/datasets")
#DATASETS_ROOT = osp.normpath(config.DATASET_BASE_PATH)
DATASETS_ROOT = osp.join(config.DATASET_BASE_PATH,config.DATASET_NAME)
...
class DFT_Dataset:
    def __init__(self, data_cfg):
        ...
        self.models_root = data_cfg["models_root"]  # BOP_DATASETS/.../models
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.dft_labor.id2obj.items() if obj_name in self.objs]
        ...
        self.scenes = [f"{i:06d}" for i in range(config.DATASET_TRAIN_NUM_TRAIN_SCENES)]
        ...
        for scene in tqdm(self.scenes):
            ...
            for str_im_id in tqdm(gt_dict, postfix=f"{scene_id}"):
                ...
                for anno_i, anno in enumerate(gt_dict[str_im_id]):
                    ...
                    if len(np.array(mask_single).shape) ==3:
                        mask_single = np.mean(mask_single,axis=2,dtype=int)
                    ...
                    if len(np.array(mask_full).shape) == 3:
                        mask_full = np.mean(mask_full,axis=2,dtype=int)
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            obj_path = osp.join( self.models_root, f"obj_{ref.dft_labor.obj2id[obj_name]:06d}.ply", # Hier auch ändern
                )
            model = inout.load_ply(
                osp.join(
                    self.models_root,
                    f"obj_{ref.dft_labor.obj2id[obj_name]:06d}.ply", # Hier auch ändern
                ),
...
def get_DFT_metadata(obj_names, ref_key):
    ...
    return meta
model_root = osp.join(DATASETS_ROOT,config.DATASET_MODELS_DIR_NAME)
#model_root = osp.normpath(f"{config.DATASET_BASE_PATH}/{config.DATASET_NAME}/{}")
#model_root = "BOP_DATASETS/{}/{}/".format(config.DATASET_NAME,config.DATASET_MODELS_DIR_NAME)
################################################################################


SPLITS_DATA = dict(
    train=dict(
        name="train", # muss gleich dem Namen des Parameters in der Zeile darüber sein
        objs=ref.dft_labor.objects,  # selected objects
        dataset_root=osp.join(DATASETS_ROOT, config.DATASET_TRAIN),
        models_root=osp.join(DATASETS_ROOT, config.DATASET_MODELS_DIR_NAME),
        xyz_root=osp.join(DATASETS_ROOT, "{}/xyz_crop".format(config.DATASET_TRAIN)),
        ...
        num_to_load=-1,
        filter_invalid=True,
        ref_key=config.DATASET_REF_KEY,
    ),
    test=dict(
        name="test", # muss gleich dem Namen des Parameters in der Zeile darüber sein
        objs=ref.dft_labor.objects,  # selected objects
        dataset_root=osp.join(DATASETS_ROOT, config.DATASET_TEST),
        models_root=osp.join(DATASETS_ROOT, config.DATASET_MODELS_DIR_NAME),
        xyz_root=osp.join(DATASETS_ROOT, "{}/xyz_crop".format(config.DATASET_TEST)),
        scale_to_meter=0.001,
        with_masks=True,  # (load masks but may not use it)
        with_depth=True,  # (load depth path here, but may not use it)
        height=config.CAMERA_HEIGHT,
...
# single obj splits
for obj in ref.dft_labor.objects:
    for split in ["train"]:
        name = "dft_{}_{}".format(obj, split)
        if split in ["train"]:
            filter_invalid = True
        elif split in ["test"]:
        ...
        if name not in SPLITS_DATA:
            SPLITS_DATA[name] = dict(
                name=name,
                objs=[obj],  # only this obj
                dataset_root=osp.join(DATASETS_ROOT, config.DATASET_TRAIN),
                models_root=osp.join(DATASETS_ROOT,config.DATASET_MODELS_DIR_NAME),
                xyz_root=osp.join(DATASETS_ROOT, config.DATASET_TRAIN),
                scale_to_meter=0.001,
                with_masks=True,  # (load masks but may not use it)
                with_depth=True,  # (load depth path here, but may not use it)
                height=config.CAMERA_HEIGHT,
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_DATA:
        used_cfg = SPLITS_DATA[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, DFT_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id=config.DATASET_REF_KEY,  # NOTE: for pvnet to determine module
        ref_key=used_cfg["ref_key"],
        objs=used_cfg["objs"],
        eval_error_types=["ad", "rete", "proj"],
        evaluator_type="bop",
        **get_DFT_metadata(obj_names=used_cfg["objs"], ref_key=used_cfg["ref_key"]),
    )
